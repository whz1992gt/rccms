...
if ($act=="countinfo")
{
	...
	$total_members=$db->get_total($total);
		$tpl='../templates/'.$_CFG['template_dir']."plus/count.ini";
...
elseif ($act=="dynamic")
{
		$result = $db->query("SELECT id,jobs_name,addtime,companyname,company_id,company_addtime FROM ".table('jobs')." ORDER BY id DESC LIMIT 10");
		...
		$result= $db->query("SELECT id,display_name,fullname,addtime FROM ".table('resume')."  ORDER BY id DESC LIMIT 10");
		while($row = $db->fetch_array($result))
		{
