...
if (isset($aset['displayorder']))
{
	...
	if ($arr[0]=="rtime")
	{
		...
		$orderbysql = " ORDER BY r.wage {$arr[1]},r.refreshtime {$arr[1]}";
	}elseif ($arr[0] == 'experience') {
		$orderbysql = " ORDER BY r.experience {$arr[1]},r.refreshtime {$arr[1]}";
	}elseif($arr[0] == 'education'){
		$orderbysql = " ORDER BY r.education {$arr[1]},r.refreshtime {$arr[1]}";
	}elseif ($arr[0] == "null"){
		$orderbysql = "";
	}else{
		$orderbysql = " ORDER BY r.refreshtime {$arr[1]}";
