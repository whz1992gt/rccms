...
if ($act=='down')
{
	...
	$smarty->display('member_personal/personal_downresume.ini');
}
elseif ($act=='interview')
{
	...
	$smarty->assign('resume_list',get_auditresume_list($_SESSION['uid']));
	$smarty->display('member_personal/personal_interview.ini');
}
...
elseif ($act=='favorites')
{
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('member_personal/personal_favorites.ini');
}
...
elseif ($act=='apply_jobs')
{
	...
	$smarty->assign('count',$count);
	$smarty->assign('resume_list',get_auditresume_list($_SESSION['uid']));
	$smarty->display('member_personal/personal_apply_jobs.ini');
}
//删除-申请的职位列表
