...
if ($act=='userprofile')
{
	...
	$smarty->display('member_personal/personal_userprofile.ini');
}
...
elseif ($act=='avatars')
{
	...
	$smarty->display('member_personal/personal_avatars.ini');
}
...
elseif ($act=='password_edit')
{
	...
	$smarty->display('member_personal/personal_password.ini');
}
//保存修改密码
...
elseif ($act=='user_status')
{
	...
	$smarty->display('member_personal/personal_user_status.ini');
}
//保存会员状态
...
elseif ($act=='user_email')
{
	...
	$smarty->display('member_personal/personal_user_email.ini');
}
elseif ($act=='user_mobile')
{
	...
	$_SESSION['send_key']=mt_rand(100000, 999999);
	$smarty->assign('send_key',$_SESSION['send_key']);
	$smarty->display('member_personal/personal_user_mobile.ini');
}
elseif ($act=='feedback')
{
	$smarty->assign('title','用户反馈 - 个人会员中心 - '.$_CFG['site_name']);
	$smarty->assign('feedback',get_feedback($_SESSION['uid']));
	$smarty->display('member_personal/personal_feedback.ini');
}
//保存用户反馈
...
elseif ($act=='pm')
{
	...
	$smarty->display('member_personal/personal_user_pm.ini');
}
elseif ($act=='pm_show')
{
	$uid=intval($_SESSION['uid']);
	...
	$smarty->assign('uid',$uid);
	$smarty->display('member_personal/personal_user_pm_show.ini');
	//统计消息
...
elseif ($act=='buddy')
{
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('member_personal/personal_user_buddy.ini');
}
