...
if($act == 'QQlogin')
{
$url="https://graph.qq.com/oauth2.0/authorize?response_type=token&client_id={$_CFG['qq_appid']}&redirect_uri={$_CFG['site_domain']}{$_CFG['site_dir']}user/connect_qq_client.php".'?act=login_check';
header("Location:{$url}");	
}
...
elseif ($act=='reg')
{
	...
	else
	{
		...
		$smarty->display('user/connect-qq.ini');
	}
}
elseif ($act=='reg_save')
{
