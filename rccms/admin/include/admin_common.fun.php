...
function makejs_classify()
{
	...
	array_unshift($education,'",不限"');
	array_unshift($resumetag,'",不限"');
	array_unshift($wage,'",不限"');
	array_unshift($jobsnature,'",不限"');
	array_unshift($scale,'",不限"');
	array_unshift($trade,'",不限"');
	array_unshift($jobtag,'",不限"');
	...
	{
	$content=iconv(QISHI_DBCHARSET,"utf-8",$content);
	}
	 if (!@fwrite($fp, trim($content))){
			exit('写入JS文件失败');
		}
